...
class ReferenceWriter(object):
    ...
    def __init__(self, reference_directory="./references/", reference_source=None, data=None, bibcode=None):
    ...
    def _create_output_file_name(self):
        ...
        try:
            ...
            if not self.bibcode:
                self.bibcode = BibcodeGenerator().make_bibcode(self.data)
                if not self.bibcode:
                    raise BibcodeCreationException("Failed to make a bibcode.")

            file_ext = refsource_dict.get(self.reference_source, "generic.txt")
